local function download(msg, success, result, matches)
  ...
  if success then
    ...
    send_large_msg(receiver, 'لطفا برای ساخت استیکر دستور "/make" را ارسال کنید', ok_cb, false)
    redis:del("file:exe")
  else
    print('Error downloading: '..msg.id)
    send_large_msg(receiver, 'مشکل ؛ عکس را دوباره ارسال کنید', ok_cb, false)
  end
end
...
f matches[1] == 'استیکر' then
     redis:set("file:exe", "waiting")
     return 'عکس را برای تبدیل شدن به استیکر ارسال کنید '
end
    return
end
return {
